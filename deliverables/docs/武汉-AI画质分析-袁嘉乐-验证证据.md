...
本文记录最终版 App 的验证证据。完整逐样本结构化记录见 `logs/武汉-AI画质分析-袁嘉乐-最终验证日志.csv`，截图清单见 `screenshots/武汉-AI画质分析-袁嘉乐-截图证据清单.md`。

## 证据文件

- `logs/武汉-AI画质分析-袁嘉乐-最终验证日志.csv`：最终验证日志，包含设备、样本路径、格式、来源、人工判断、分析模式、原始尺寸、分析尺寸、是否降采样、分析耗时、四项子分、综合分、匹配状态、局限说明、截图文件名。
- `screenshots/武汉-AI画质分析-袁嘉乐-截图证据清单.md`：最终截图索引，按 `模拟器/` 和 `红米K70E/` 分开列出。
...
| 证据类型 | 对应文件 | 评审应该看什么 |
|---|---|---|
| 日志 | `logs/武汉-AI画质分析-袁嘉乐-最终验证日志.csv` | 最完整的逐样本原始记录。每一行对应一次验证，包含设备、样本、格式、来源、尺寸、是否降采样、耗时、四项子分、综合分、匹配状态和截图路径。 |
| 截图 | `screenshots/武汉-AI画质分析-袁嘉乐-截图证据清单.md`、`screenshots/模拟器/`、`screenshots/红米K70E/` | 截图索引和实际运行画面。用于证明 App 不是只在文档里存在，而是在模拟器和 Redmi K70E 上完成选图、预览和分析。 |
| 数据 | 本文的“样本覆盖”“模拟器完整验证摘要”“Redmi K70E 真机验证摘要” | 从 CSV 中整理出的评审友好表格，展示类别覆盖、格式覆盖、大图处理、总分和主要说明。 |
| 对比 | 本文的“人工判断 / 综合分 / 匹配状态 / 主要说明”列，以及 `deliverables/docs/武汉-AI画质分析-袁嘉乐-反例分析.md` | 对比人工判断和 App 输出是否一致。`Match` 表示基本一致，`Partial` 表示部分一致，`Mismatch` 表示明显不一致；反例文档进一步解释为什么会错。 |
...
最终采用两个反例，详见 `deliverables/docs/武汉-AI画质分析-袁嘉乐-反例分析.md`：
...
完整逐项数据以 `logs/武汉-AI画质分析-袁嘉乐-最终验证日志.csv` 为准。
